...
def hp_contacts(path, sequence, Epsilon=1):
    """
...
def partition_function(all_paths, sequence, Beta=Beta, Epsilon=1):
    Z = 0
    for path in all_paths:
        Z += math.exp(-Beta * hp_contacts(path, sequence, Epsilon))
    return Z

def probability(path, sequence, Z, Beta=Beta, Epsilon=1):
    return math.exp(-Beta * hp_contacts(path, sequence, Epsilon)) / Z
...
def average_rg(paths, sequence, Epsilon, Beta):
    Z = 0.0
    weighted = 0.0
...
def entropy_term(path, sequence, Z, Beta=Beta, Epsilon=1):
    """
    Returns p ln p for one microstate.
    """
    p = probability(path, sequence, Z, Beta, Epsilon)
    return p * math.log(p)

def entropy_from_definition(expected_energy, Z, Beta=Beta):
    """
    S = beta <E> + ln Z
    """
    return Beta * expected_energy + math.log(Z)

def energies_of_paths(all_paths, sequence, Epsilon=1):
    energies = []
    for path in all_paths:
        energies.append(hp_contacts(path, sequence, Epsilon))
    return energies

def macrostates(all_paths, sequence, Epsilon=1):
    """
    Returns a Counter of energy degeneracies.
    """
    return Counter(energies_of_paths(all_paths, sequence, Epsilon))

def lowest_energy_microstates(paths, sequence, Epsilon=1):
    """
    Return:
...
def analyze_paths(all_paths, sequence, Beta=Beta, Epsilon=1):
    """
    Run the same analysis from class and return the results in a dictionary.
    """
    Z = partition_function(all_paths, sequence, Beta, Epsilon)
    ...
    for path in all_paths:
        ...
        sum_end2end += r
        sum_rg += rg
        expected_end2end += p * r
        expected_energy += p * e
        entropy1 -= entropy_term(path, sequence, Z, Beta, Epsilon)
        energies.append(e)
    ...
    results = {
        ...
        "S1": entropy1,
        "S2": entropy_from_definition(expected_energy, Z, Beta),
        "macrostates": macro
    }
